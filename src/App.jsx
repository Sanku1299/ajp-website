...
export default function App() {
  ...
  return (
    <div className="relative min-h-screen bg-[#020617] text-white flex flex-col justify-start">
      ...
      {/* Page Content: ONLY Hero & Manifesto Sections */}
      <main className="flex-1">
        ...
        {/* Section 2: Manifesto Section */}
        <div ref={manifestoRef}>
          <ManifestoSection />
        </div>
      </main>

      {/* Cinematic Watch Trailer Modal (Simulated Protest Trailer) */}
      {trailerOpen && (
        <div 
        ...
        >
          <div 
            className="w-full max-w-4xl aspect-video bg-black border border-brand-orange/40 rounded-lg shadow-[0_0_50px_rgba(255,85,0,0.3)] relative overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <button 
              className="absolute top-4 right-4 text-white hover:text-brand-orange z-10 transition-colors cursor-pointer bg-black/60 p-2 rounded-full"
              onClick={() => setTrailerOpen(false)}
            >
              <X size={20} />
            </button>
            <iframe 
              ...
              title="AJP Rebellion Cinematic Trailer"
              frameBorder="0"
      ...
      {joinOpen && (
        ...
        >
          <div 
            className="w-full max-w-md bg-brand-navy-card/95 border border-brand-orange/30 p-8 rounded-lg shadow-[0_0_50px_rgba(255,85,0,0.4)] relative"
            onClick={(e) => e.stopPropagation()}
          >
            <button 
